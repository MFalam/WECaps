...
def create_options():
    parser = OptionParser()
    parser.add_option("-l", "--lr", dest="learning_rate", default=LEARNING_RATE, type="float",
                      help="learning rate")
    parser.add_option("-d","--decoder", dest="decoder", default=DECODER,
                      help="Decoder structure 'FC' or 'Conv'")
    parser.add_option("-b", "--batch_size", dest="batch_size", default=BATCH_SIZE, type="int")
    parser.add_option("-e", "--epochs", dest="epochs", default=EPOCHS, type="int",
                      help="Number of epochs to train for")
    parser.add_option("-f", "--file", dest="filepath", default="", type="string",
                      help="Name of the model to be loaded")
    parser.add_option("-g", "--use_gpu", dest="use_gpu", default=USE_GPU, action="store_false",
    ...
    parser.add_option("-a", "--alpha", dest="alpha", default=ALPHA, type="float",
                      help="Alpha constant from paper (Amount of reconstruction loss)")
    parser.add_option("--dataset", dest="dataset", default=DATASET, help="Set wanted dataset. Options: [mnist, small_norb,cifar10]")
    parser.add_option("-r", "--routing", dest="routing_iterations", default=ROUTING_ITERATIONS, type="int",
